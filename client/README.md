...
This is a component that utilizes the render-prop pattern to inject the `web3`, `accounts`, and `contract` instance objects into a given function. When these objects are loading, it will render a loading function that is expected to return a React component.

For an example of how to use it, look at the `accounts` and `dapp` pages.
...
This function requires `web3` to be passed in. It uses `truffle-contract` to initialize and return a contract instance. This function is used by `Web3Container.js`.

You would edit this file if you wanted to change the contract your dapp required. If you wanted to require multiple contracts, I recommend making a copy of this file with your changes and then updating the `Web3Container` accordingly.

In the future, we might want to make this more reusable.

### `getAccounts.js`

This simply wraps `web3.eth.getAccounts` into a Promise so we can use it cleanly inside `Web3Container.js`.
